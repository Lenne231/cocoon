...
type XmlNode =
    | IndexRoute of XmlNode list
    | SegmentRoute of string * XmlNode list
...
let routeIndex (contents: XmlNode list) =
  IndexRoute contents

let routeSegment segment (contents: XmlNode list) =
  SegmentRoute (segment, contents)
...
let tag (tagName: string) (attributes: XmlAttribute list) (contents: XmlNode list) =
    ParentNode((tagName, attributes), contents)

let voidTag (tagName: string) (attributes: XmlAttribute list) = VoidElement(tagName, attributes)
...
let renderAttributes (write: string -> unit) attr =
  for a in attr do
    write " "
    match a with
    | XmlAttribute.Boolean n -> 
        write n
    | XmlAttribute.KeyValue (k, v) ->
        write k
        write "="
        write "\""
        write v
...
let rec renderNode (path: string list) (write: string -> unit) node=
  match node with
  | XmlNode.IndexRoute children ->
      if path.IsEmpty
      then for child in children do renderNode path write child
  | XmlNode.SegmentRoute (segment, children) ->
      match path with
      | s::rest when s = segment -> 
          for child in children do 
            renderNode rest write child
      | _ -> ()
  | XmlNode.Text s -> write s
  | XmlNode.VoidElement (tag, attr) ->
      write "<"
      write tag
      renderAttributes write attr
      write " />"
  | XmlNode.ParentNode ((tag, attr), children) ->
      write "<"
      write tag
      renderAttributes write attr
      write ">"
      ...
      for child in children do
        renderNode path write child

      write "</"
      write tag
      write ">"

let rec getPaths path node =
  match node with
  | XmlNode.Text _ | XmlNode.VoidElement _ -> Seq.empty
  | XmlNode.IndexRoute _ -> Seq.empty
  | XmlNode.ParentNode (_, contents) ->
      seq {
        yield []
        for node in contents do
          yield! getPaths path node
      }
  | XmlNode.SegmentRoute (segment, contents) ->
      let path = path |> List.append [segment]
      seq {
        yield path
        for node in contents do
          yield! getPaths path node
...
let render rootPath root =
    ...
    for segments in paths do
      ...
      renderNode segments (fun s -> writer.Write s) root 
