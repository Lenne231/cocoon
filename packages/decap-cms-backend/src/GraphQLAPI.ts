...
export default class GraphQLAPI extends API {
  ...
  async getPullRequestAuthor(pullRequest: any /*Octokit.PullsListResponseItem*/) {
  ...
  async getPullRequests(
    ...
    predicate: (pr: any /*Octokit.PullsListResponseItem*/) => boolean,
  ) {
  ...
  async createBranchAndPullRequest(branchName: string, sha: string, title: string) {
    ...
    const { pullRequest } = data!.createPullRequest;
    return transformPullRequest(pullRequest) as unknown as any /*Octokit.PullsCreateResponse*/;
  }
